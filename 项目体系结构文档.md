...
There are 3 states of lifestyle 1. initialization 2. update 3. destruction.
...
1. Difficulty in asynchronous programming between ReactJS and Server.

   When data is loaded via Model to View, Model Layer is responsible to set the data in **state** of ReactJS and **render** of ReactJS will render it to the web page. When I/O operations are performed, the results of them are handled by callback functions instead of blocking the current execution flow. During this procedure, programmers need to write a bunch of asynchronous processes which could be hard to debug and maintain.

2. Limited number of compatible third-party components.

   React focuses on the View Layer of the MVC model, which means that other frameworks are needed in developing a whole project. Nevertheless, when it comes to the number of compatible third-party frameworks like Reduxm and route, React is no longer advantageous over its competitors such as Angular. What's more, users can’t know for sure that community-built modules will be supported by the next releases of the framework.
